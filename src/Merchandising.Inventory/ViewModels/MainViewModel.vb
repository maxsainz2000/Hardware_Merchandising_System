' Merchandising.Inventory.ViewModels.MainViewModel
'
' P4-12: the composition root for the Inventory client. Owns the one
' MerchandisingApiClient the window uses, the sign-in/sign-out screen, and
' the five workspace tabs (stock, receive, counts, adjustments, low-stock) -
' each its own view model, each talking to the API directly and never to
' each other's state, the same shape Merchandising.Procurement.ViewModels.
' MainViewModel (P3-07) already establishes for that client.
'
' Session handling is identical to Procurement's: the token lives in
...
' from the refusal itself rather than by polling. P1-15's SpikeViewModel -
' one window, one screen - is retired by this card, not extended.
...
Namespace ViewModels
    ...
    ''' <summary>Composition root and sign-in/sign-out state for the Inventory client.</summary>
    Public NotInheritable Class MainViewModel
        ...
        Public Sub New(client As MerchandisingApiClient)
            ...
            Stock = New StockBrowseViewModel(_client)
            Receive = New ReceivingViewModel(_client)
            Counts = New StockCountsViewModel(_client)
            Adjustments = New AdjustmentsViewModel(_client)
            LowStock = New LowStockReviewViewModel(_client)

            SignOutCommand = New AsyncRelayCommand(AddressOf SignOutAsync, Function() Not IsBusy AndAlso IsSignedIn)

        End Sub

        Public ReadOnly Property Stock As StockBrowseViewModel
        Public ReadOnly Property Receive As ReceivingViewModel
        Public ReadOnly Property Counts As StockCountsViewModel
        Public ReadOnly Property Adjustments As AdjustmentsViewModel
        Public ReadOnly Property LowStock As LowStockReviewViewModel
