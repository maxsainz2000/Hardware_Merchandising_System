' Merchandising.Tests.Unit.InventoryLayoutTests
'
' P4-12's own Done-when box: "keyboard navigation and focus order work at
' 1366x768 and 125% scaling - asserted by a test, not by a sentence. Extend
' ProcurementLayoutTests' three assertions to this window." This suite is
' that extension, adapted line-for-line from ProcurementLayoutTests (P3-07)
' against Merchandising.Inventory.MainWindow instead.
'
...
Imports Merchandising.Inventory
Imports Merchandising.Inventory.ViewModels
Imports Microsoft.VisualStudio.TestTools.UnitTesting

''' <summary>
''' Layout and focus-order acceptance for the Inventory client's one window,
''' against the smallest desktop the spec supports.
''' </summary>
<TestClass>
<DoNotParallelize>
Public NotInheritable Class InventoryLayoutTests
    ...
    ''' Signed in as "inventoryclerk" - spec section 9 assigns this client to the
    ''' Inventory Clerk role - not that the role matters for a pure layout pass.
    ''' </summary>
    Private Shared Async Function SignedInViewModelAsync() As Task(Of MainViewModel)
        ...
        viewModel.Username = "inventoryclerk"
    ...
    Private Shared Function LoginJson() As String
        ...
        Return JsonSerializer.Serialize(New With {
            .token = "P4-12-LAYOUT-FIXTURE-TOKEN",
