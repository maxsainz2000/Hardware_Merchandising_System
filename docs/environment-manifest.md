...
> **Gate outcome 2026-08-22 — both outstanding items carried to the Phase 6 gate by ADR-016.** The Phase 1 gate ran against commit `41ee833`, returned FAIL on exactly the rows above, and was closed on its engineering criteria by amendment rather than by ticking anything. **Nothing in this section became true; its deadline moved.** P0-02 and P0-05 are now exit criteria of the **Phase 6** gate, where `plan.md` §7's clean-installation criterion absorbs both — installing on a classmate's laptop from the guide alone *is* the survey and *is* the rehearsal.
>
> Two things did change here, and both were stale rather than newly done: the phpMyAdmin box closed at P0-03 on 2026-08-21, and the off-host backup destination closed at P1-17 on 2026-08-22. The P1-20 audit table above lists three gaps and never counted the backup box — corrected in the checklist below.
>
> **The one item still worth doing today, out of band:** ask the three classmates whether they hold local administrator rights. It needs no machine in the room and it is the only carried item that fails late and unfixably.
...
- [x] XAMPP stripped to MariaDB only — `evidence/phase-0/xampp-services.txt` (PowerShell process/service/port output substituted for a screenshot, by agreement). *Cross-machine phpMyAdmin unreachability **verified 2026-08-21** from two lab clients, with Apache deliberately running so the test is not vacuous — `evidence/phase-1/p1-10-cross-machine-denials.txt` (P0-03). Read the claim precisely: unreachable at the **host firewall** layer; Apache's own `Require local` was never exercised because no HTTP response reached either client.*
- [x] MariaDB bound to loopback, config excerpt attached — `evidence/phase-0/mariadb-config.txt`
- [x] Backup directory **and** off-host destination chosen and writable — **completed 2026-08-22 at P1-17.** The off-host half is no longer outstanding: the scheduled run wrote a byte-identical copy to the `MERCHBACKUP` volume, SHA-256 independently recomputed on both copies and matching the recorded value (`evidence/phase-1/p1-17-backup-success.log`). The original wording is kept below because its reasoning still stands and still has a Phase 6 half. ~~**half done.**~~ `C:\MerchandisingBackups` created, ACL-restricted and write-proven (`evidence/phase-0/p0-07-network-and-backup.txt`) — but that is a *lab* path. Under ADR-012 both halves must become install-time configuration, so that a classmate's machine is not required to have a `C:\MerchandisingBackups` the author happened to create.
